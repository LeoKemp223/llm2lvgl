...
4. Run `tools/m1-page-flow.sh loop <task-id> <iteration>`.
...
## Commands
```bash
tools/m1-page-flow.sh build token
tools/m1-page-flow.sh screenshot token
tools/m1-page-flow.sh validate token
tools/m1-page-flow.sh run token
tools/m1-page-flow.sh loop token 1
```

## Task Format
- `page_id`: artifact folder id
- `page_name`: human-readable label
- `entry_page`: page registry id used by `M1_PAGE`
...
1. Start with `tools/m1-page-flow.sh loop <task-id> 1`
...
- Use stable page ids through `M1_PAGE` or `--page`.
